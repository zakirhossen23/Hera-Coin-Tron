...
contract HeraCoin {
        ...
        returns (uint256)
    {
        ...
        AllTokensGift[_TokenGiftIds] = [_TokenID, _GiftURI];
        _TokenGiftIds++;
        return _TokenGiftIds;
    }
        ...
        returns (uint256)
    {
        for (uint256 i = 0; i < _tokenIds; i++) {
            if (
                keccak256(bytes(_tokenURIs[i][0])) ==
                keccak256(bytes(_tokenURI))
            ) {
                return i;
            }
    ...
    function _setEventURI(
        ...
        _eventURIs[eventId] = [_eventWallet, _eventURI, "False"];
    }
    ...
    function DistributeToken(uint256 _eventId) public {
    ...
    function GetNFTsByUserAddrs(string memory _user)
        public
        view
        returns (string[] memory)
    {
        ...
        for (uint256 i = 0; i < _UserTokenIds; i++) {
            if (
                keccak256(bytes(AllUserToken[i][0])) == keccak256(bytes(_user))
            ) {
                _TemporarySearch++;
            }
        }
        ...
        for (uint256 i = 0; i < _UserTokenIds; i++) {
            if (
                ...
                _SearchedStoreUser[_SearchIds] = tokenURI(
                    strToUint(AllUserToken[i][1])
                );
                _SearchIds++;
            }
        }
    ...
    function GetGiftedFromToken(string memory _TokenID)
        public
        view
        returns (string memory)
    {
    ...
    function GetGiftURIFromToken(string memory _TokenID)
        public
        view
        returns (string memory)
    {
    ...
    function UnWrapGift(string memory _TokenID) public returns (uint256) {
        for (uint256 i = 0; i < _UserTokenIds; i++) {
            if (
                keccak256(bytes(AllUserToken[i][1])) ==
                keccak256(bytes(_TokenID))
    ...
    function createUserToken(string memory _user, string memory _tokenid)
        private
    {
        AllUserToken[_UserTokenIds] = [_user, _tokenid, "False"];
        _UserTokenIds++;
    }
    ...
    function reset_all() public {
      ...
      _EventTokenSearchIds = 0;
      _UserTokenIds = 0;
      for (uint256 i = 0; i < _EventTokenIds; i++)      delete AllEventTokens[i];
      for (uint256 i = 0; i < _TokenBidIds; i++)        delete AllTokensBids[i];
      for (uint256 i = 0; i < _TokenGiftIds; i++)       delete AllTokensGift[i];
      for (uint256 i = 0; i < _UserTokenIds; i++)       delete AllUserToken[i];
      for (uint256 i = 0; i < _bidIds; i++)          	delete _bidURIs[i];
      for (uint256 i = 0; i < _tokenIds; i++)          	delete _tokenURIs[i];
      for (uint256 i = 0; i < _eventIds; i++)     		delete _eventURIs[i];    
      for (uint256 i = 0; i < _eventIds; i++)     		delete _eventRaised[i];      
